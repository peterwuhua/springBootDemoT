					...
					<div id="tab-2" class="tab-pane">
                   		<div class="panel-body">
                             <form action="" method="post" id="listForm">
								...
								<input type="hidden" name="disPage" id="disPage" value="souce">
								<div class="col-sm-9">
									<button class="btn btn-info" href="javascript:;" onclick="jqgridExportOther('exportOther.do','bi-customer-souce-export.xls','客户分布统计.xls',0)">导出</button>
								</div>
...
function seachForm(){
	...
	var souceList = new Array(),
		numList = new Array();
	$.ajax({
		type : "post",
		async : true,            //异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
		url : "ajaxSeachSouce.do",    
...
//刷新echarts
function refreshData(souce,num){
	if(!myChart){
		return;
	}
	//更新数据
...
function seeData(souce){
	layer.open({
		type: 2,
		area: ['800px', '600px'],
		fix: false, //不固定
		maxmin: true,
		content: 'seeData.do?souce='+souce+'&customerType='+$('#customerType').val()+'&saler='+$('#saler').val()+'&areaPath='+$('#areaPath').val()
					+'&buildDate='+$('#buildDate').val()+'&industry='+$('#industry').val()
	});
}
</script>
</html>