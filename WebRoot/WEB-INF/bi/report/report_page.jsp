...
<body class="gray-bg">
			...
			<div class="ibox-content">
				<form action="gridPage.do" method="post" id="listForm" class="form-horizontal m-t">
					<div class="form-group col-md-3">
                          <label class="col-md-4 control-label" >报告编号：</label>
                          <div class="col-md-8" >
                              <input id="reportNo" name="reportNo" value="${vo.reportNo}" type="text" class="form-control" placeholder="报告编号">
                          </div>
                    </div>
                    <div class="form-group col-md-3">
                    ...
                    <div class="form-group col-md-3">
                      ...
                      <div class="form-group col-md-6" id="data_5">
                          <label class="col-md-2 control-label" style="width: 81px;">报告日期：</label>
	...
	$(function() {
		var url = 'gridData.do';
		var colNames = ['','报告编号','任务编号','客户名称','样品名称','检测性质','报告日期','当前进度','操作'];
		var colModel = [ 
			{
				name : 'taskVo.id',
				index : 'task.id',
	...
	function gridComplete() {
		...
		for (var i = 0; i < ids.length; i++) {
			...
			var be = '<a class="btn btn-outline btn-success btn-xs" title="日志" href="javascript:fnSelectProcess(\''+rData['taskVo.id']+'\',\''+rData.status+'\')">日志</a>';
	...
	function fnShow(id,no){
		...
		var url='/bus/report/show.do?id='+id;
		parent.layer.open({
			title:'报告【'+no+'】',	
			type: 2,
			area: ['1000px','85%'],
	...
	function fnSearch(){
		...
		$.extend(postData, {'reportNo':$('#reportNo').val(),'taskVo.no':$('#no').val(),'custVo.custName':$('#custName').val(),'taskType':$('#taskType').val()
