...
<script type="text/javascript">
	...
	function fnEdit(){
		...
		parent.layer.open({
			...
			fix: false, //不固定
			maxmin: true,
			content: '${basePath}init/pstandItem/edit.do?sampTypeId=${vo.sampTypeId}&standId=${vo.standId}&sampTypeName='+encodeURI(sampTypeName),
			btn: ['确定','关闭'],
	...
	$(function() {
		var url = 'gridData.do?standId=${vo.standId}';
		var editurl='gridEdit.do';
		var colNames =[];
		var colModel =[];
		var sampType='${vo.sampType}';
		if(sampType=='气'){
			colNames = ['样品分类','检测项目','指标','分类','其他分类','速率限值','浓度限值',''];
			colModel = [ 
			{
				name : 'sampTypeName',
				index : 'sampTypeName',
			...
			colNames = ['样品分类','检测项目','指标','分类','其他分类','昼间','夜间',''];
			colModel = [ 
			{
				name : 'sampTypeName',
				index : 'sampTypeName',
			...
			colNames = ['样品分类','检测项目','指标','分类','其他分类','限值',''];
			colModel = [ 
			{
				name : 'sampTypeName',
				index : 'sampTypeName',
	...
	function editRow(id){
		parent.layer.open({
			...
			fix: false, //不固定
			maxmin: true,
			content: '${basePath}init/pstandItem/editData.do?id='+id,
			btn: ['更新','关闭'],
			btn1: function(index, layero) {
				var iframeWin=layero.find('iframe')[0];
				iframeWin.contentWindow.fnSelect();
