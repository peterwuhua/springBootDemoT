	...
	<div class="ibox float-e-margins">
		<div class="ibox-content" style="padding: 0px;">
			<form action="" method="post" >
				...
				<div id="im_div" style="width: 690px;float:left;overflow-y:auto;">
...
var global_ids='${vo.ids}';
	...
	function generateTabRow(row,index){
		if(!hasChecked(row.id)){
			...
			$('#im_tb').append($('<tr key='+index+'>')
					...
					.append('<td key="'+row.id+'"><select name="methodId" class="form-control">'+opt+'</select></td>')
	...
	function fnSelect(){
		...
		$('#im_tb').find("tr").each(function(){
			...
			var methodId=$(this).find('select[name="methodId"]').val();
			var p=$(this).find('input[name="price"]').val();
			itemIdArr.push(itemId);
			itemNameArr.push(itemName);
			imIdArr.push(itemId+'-'+methodId);
			price+=parseFloat(p);
		});
		var data={};
		data.id=itemIdArr.join(',');
		data.name=itemNameArr.join(',');
		data.im=imIdArr.join(';');
		data.price=price;
		return data;
	}
</script>
