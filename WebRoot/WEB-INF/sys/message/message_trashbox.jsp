...
<body class="gray-bg">
	<div class="row">
		<div class="col-sm-2">
			<jsp:include page="message_left.jsp?flag=trash" />
		</div>
		<div class="col-sm-10">
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="mail-tools tooltip-demo m-t-md">
						...
						<form method="post" action="trash.do" class="pull-right mail-search">
							...
							<tbody>
								<c:forEach items="${list}" var="e">
									<tr class="${1==e.readFlag?'read':'unread'} ">
										...
										<td class="mail-ontact"><a href="detail.do?detailId=${e.id }&flag=trash">${e.messageVo.senderVo.userVo.name}【${e.messageVo.senderVo.loginName}】</a> <c:if test="${1!=e.readFlag}">
												<span class="label label-danger pull-right">未读</span>
											</c:if></td>
										<td class="mail-subject"><a href="detail.do?detailId=${e.id }&flag=trash">${e.messageVo.subject}</a></td>
	...
	<script> 
		...
		function fnSubmit(t){
			...
			$('form').attr('action','updatePosition.do?position='+t);
			$('form').submit();
	    }
		...
		function fnDrop(){
			...
			$("#ids").val(getSelectIds());
			$('form').attr('action','drop.do?from=trash&flag=trash');
			$('form').submit();
	    }
    </script>
</body>
