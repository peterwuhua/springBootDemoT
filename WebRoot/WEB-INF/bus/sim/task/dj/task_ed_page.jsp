...
<body class="gray-bg">
	...
	function loadComplete(data) {
		...
		for (var i = 0; i < ids.length; i++) {
			...
			be = '<a class="btn btn-outline btn-success btn-xs" title="日志" href="javascript:fnSelectProcess(\''+rData.id+'\',\''+rData.status+'\')">进度</a>';
	...
	function updateStop(){
		...
		layer.confirm('确认要终止吗?', {icon:3, title:'系统提示'}, function(index){
			$.ajax({ 
				url:"${basePath}bus/sim/project/taskdj/update2Stop.do",
	...
	function fnShow(id){
		parent.layer.open({
			...
			fix: false, //不固定
			maxmin: true,
			content: '/bus/sim/project/taskdj/show.do?id='+id
		});
	}
</script>
<script>
