...
<!--Web Uploader文件上传 引入CSS-->
<link rel="stylesheet" type="text/css" href="${basePath}h/css/plugins/webuploader/webuploader.css">
<link rel="stylesheet" type="text/css" href="${basePath}h/css/demo/webuploader-demo.css" />
</head>
<body class="gray-bg">
	<div id="uploader">
		<div class="queueList">
	...
	<script src="${basePath}h/js/plugins/webuploader/webuploader.min.js"></script>
	<script type="text/javascript">
		$(function() {
			...
			if (!WebUploader.Uploader.support('flash') && WebUploader.browser.ie) {
				...
				if (flashVersion) {
					(function(container) {
						...
						var swf = '${basePath}h/plugins/webuploader/image-upload/expressInstall.swf';
						// insert flash object
			...
			uploader = WebUploader.create({
				...
				dnd : '#dndArea',
				paste : '#uploader',
				swf : '${basePath}h/plugins/webuploader/Uploader.swf',
				chunked : false,
				server : '${basePath}bus/archiveFile/fileUpload.do',
				// runtimeOrder: 'flash',

				accept : {
					title : 'PDF文件',
					extensions : 'jpg,jpeg,png,pdf,doc,docx,rar,pdf,zip',
					mimeTypes : '.rar,.doc,.docx,.pdf,.jpg,.png,.jpeg,.zip'
				},
			...
			uploader.options.formData.uid = '${vo.id}';
			// 拖拽时不接受 js, txt 文件。
				...
				if (file.getStatus() === 'invalid') {
					...
					uploader.makeThumb(file, function(error, src) {
						...
						if (isSupportBase64) {
							...
							$.ajax('${basePath}bus/archiveFile/fileUpload.do', {
								method : 'POST',
								data : src,
								dataType : 'json'
							}).done(function(response) {
								if (response.result) {
									img = $('<img src="'+response.object+'">');
									$wrap.empty().append(img);
								} else {
									$wrap.text("预览出错");
								}
