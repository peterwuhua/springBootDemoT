							...
							<c:forEach items="${vo.sampList}" var="e" varStatus="v">
								<tr>
						    		...
						    		<td>
							 		...
							 		<td>
							 			 <div class="input-group" style="width: 100%">
								 			<input type="text" class="form-control" name="sampList[${v.index}].recordVo.demo1" value="${e.recordVo.pl}">
							 		...
							 		<td>
							 			<div class="input-group" style="width: 100%">
								 			<input type="text" id="val_${v.index}_1" class="form-control "  name="sampList[${v.index}].recordVo.v1" value="${e.recordVo.v1}" onchange="countVal(${v.index});">
								 			<span style="display: table-cell;" class=" btn btn-xs glyphicon glyphicon-arrow-down" aria-hidden="true" onclick="copyCtVals(this);return false;"></span>
							 			</div>
							 		</td>
							 		<td>
							 			<div class="input-group" style="width: 100%">
								 			<input type="text" id="val_${v.index}_2" class="form-control "  name="sampList[${v.index}].recordVo.v2" value="${e.recordVo.v2}" onchange="countVal(${v.index});">
								 			<span style="display: table-cell;" class=" btn btn-xs glyphicon glyphicon-arrow-down" aria-hidden="true" onclick="copyCtVals(this);return false;"></span>
							 			</div>
							 		</td>
							 		<td>
							 			<div class="input-group" style="width: 100%">
								 			<input type="text" id="val_${v.index}_3" class="form-control "  name="sampList[${v.index}].recordVo.v3" value="${e.recordVo.v3}" onchange="countVal(${v.index});">
								 			<span style="display: table-cell;" class=" btn btn-xs glyphicon glyphicon-arrow-down" aria-hidden="true" onclick="copyCtVals(this);return false;"></span>
							 			</div>
							 		</td>
							 		<td>
							 			 <div class="input-group" style="width: 100%">
								 			<input type="text" id="Laeq_${v.index}" class="form-control" name="sampList[${v.index}].recordVo.avg1" value="${e.recordVo.avg1}">
							 			 </div>
							 		</td>
							 		<td>
							 			 <div class="input-group" style="width: 100%">
								 			<input type="text" id="Lex_${v.index}" class="form-control" name="sampList[${v.index}].recordVo.avg2" value="${e.recordVo.avg2}">
							 			 </div>
							 		</td>
							 		<td>
							 			<div class="input-group" style="width: 100%">
...
function countVal(index){
	var v1=parseFloat($('#val_'+index+'_1').val());
	var v2=parseFloat($('#val_'+index+'_2').val());
	var v3=parseFloat($('#val_'+index+'_3').val());
	var v=0;
	var n=0;
	if(!isNaN(v1)){
		v+=v1;
	...
	$('#Laeq_'+index).val(fmtVal(v,2));
	var wh=parseFloat($('#wh_'+index).val());
	if(isNaN(wh)){
		layer.alert('请检查接触时间！', {icon: 2,title:'系统提示',shadeClose: true});
		return false;
	}
	var lex=v+10*Math.log(wh/8);
	$('#Lex_'+index).val(fmtVal(lex,2));
}
