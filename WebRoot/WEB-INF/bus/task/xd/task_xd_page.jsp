...
<body class="gray-bg">
			...
			<div class="ibox-content">
				<form action="gridPage.do" method="post" id="listForm">
					...
					<div>
						...
						<a class="btn btn-primary" href="javascript:;" onclick="fnEdit();"><i class="fa fa-pencil-square-o" aria-hidden="true"> 批量下达</i></a>
					</div>
	...
	$(function() {
		var url = 'gridData.do';
		var colNames = ['任务编号','受检单位','检测类型','样品名称','样品数量','要求完成日期','操作'];
		var colModel = [ 
			{
				name : 'no',
				index : 'no',
	...
	function gridComplete() {
		...
		for (var i = 0; i < ids.length; i++) {
			...
			be = '<a class="btn btn-outline btn-success btn-xs" title="编辑" href="javascript:openEditPage(\'edit.do?ids='+ids[i]+'\')">编辑</a>';
	...
	function fnEdit(){
		...
		if(selectIds.length<1){
			layer.msg('请选择要下达的记录', {icon: 0,time: 3000});
			return false;
		}else{
			$("#ids").val(selectIds);
		}
	...
	function formatTask(cellValue,options,rowObject){
		var ops='<a href="javascript:void();" onclick="fnTask(\''+rowObject.id+'\',\''+cellValue+'\');">'+cellValue+'</a>';
		if(rowObject.jj =='是'){
