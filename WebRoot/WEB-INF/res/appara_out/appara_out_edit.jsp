...
<body>
							...
							<tr>
								...
								<td>
									<div class="input-group">
										<input type="text" id="appName" name="apparaVo.name" class="form-control required" validate="required" value="${vo.apparaVo.name}" autocomplete="off" onclick="fnSelectApp()">
										<input type="hidden" id="appId" name="apparaVo.id"  value="${vo.apparaVo.id}"/> 
	...
	function fnSelectApp(){
		...
		parent.layer.open({
			...
			fix: false, //不固定
			maxmin: true,
			content: '${basePath}res/appara/select.do?id='+id,
			btn: ['确定','取消'], //按钮
	...
	function submitSave(){
		...
		var url='${basePath}res/apparaOut/addData.do';
		var id=$('#id').val();
		if(id!=''){
