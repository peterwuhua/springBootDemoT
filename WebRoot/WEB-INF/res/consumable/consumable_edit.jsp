...
<body>
					...
					<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
						<tbody>
							<tr>
								...
								<td class="width-35"><input type="text" id="name" class="form-control required" validate="required" name="name" value="${vo.name }" placeholder="请输入名称"></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td class="width-35">
									<%-- <input id="type" placeholder="请输入类型"  class="form-control" name="type" type="text" value="${vo.type }" /> --%> 
									<select id="type" class="form-control" name="type" type="text" onclick="typeSelect();">
										<option value="${vo.type}">${vo.type}</option>
								</select>
								</td>
								<td class="width-15 active"><label class="pull-right">型号:</label></td>
								<td class="width-35"><input id="model" placeholder="请输入型号" class="form-control" name="model" type="text" value="${vo.model }" /></td>
							</tr>
							<tr>
	...
	$(function(){
		...
		function typeSelect() {
			$.ajax({
				url : '${basePath}sys/code/ajaxCodeContent.do?code=res-hctype',
				datatype : "json",
