    ...
    var MagicSuggest = function(element, options)
    {
        ...
        this.setValue = function(data)
        {
            var values = $.isArray(data) ? data : [data],
                items = [];
            var found = false;
            var _datavalue=this.combobox.children();
            $.each(values, function(index, value) {
	            $.each(_datavalue, function(index, suggestion) {
	                var obj = $(suggestion).data('json');
	                	if(obj[cfg.valueField] == value){
	                		items.push(obj);
	                	}
	                });
	            if(!found){
        ...
        var self = {
                ...
                if(cfg.data !== null) {
                    if(typeof(cfg.data) === 'string' && cfg.data.indexOf(',') < 0) { // get results from ajax
                        ...
                        if(cfg.data.length > 0 && typeof(cfg.data[0]) === 'string') { // results from array of strings
                        	_cbData =  self._displaySuggestions(self._sortAndTrim(self._getEntriesFromStringArray(cfg.data)));
                        }
                        else { // regular json array
                        	_cbData =  self._displaySuggestions(self._sortAndTrim(cfg.data));
                        }
                    }
                }
            },
                ...
                $.each(items, function(index, value) {
                    ...
                    var resultItemEl = $('<div/>', {
                        'class': 'ms-res-item ' + (isGrouped ? 'ms-res-item-grouped ':'') +
                            ...
                            (value['state'] === 'yzf' || value['state'] === 'fz' ? ' ms-res-stop' : '') +
                            (value['state'] === 'bfyx' ? ' ms-res-volid' : '') ,
                         'title': ''+(value['state'] === 'yzf'? '已作废' : '') +
                 		 (value['state'] === 'fz'? '废止' : '') +
                 		 (value['state'] === 'bfyx'? '部分有效' : '') ,
            ...
            _renderSelection: function() {
                ...
                $.each(_selection, function(index, value){
                    ...
                    if(asText === true) {
                        ...
                        //editItemEl.click($.proxy(handlers._onEditTagTriggerClick, ref));
						editItemEl.click($.proxy(handlers._onEditTagTriggerClick, ref));
                ...
                if(cfg.selectionPosition === 'inner') {
                    ...
                    console.log(ms.selectionContainer.height());
            ...
            _sortAndTrim: function(data) {
                ...
                // take out the ones that have already been selected 取出已选择的那些
        ...
        var handlers = {
            ...
            _onEditTagTriggerClick: function (e) {
                var d=$(e.currentTarget).data('json');
                // alert(d.name + "\r\n" + d.id);
                age = prompt("修改:", d.name); 
