...
<body class="gray-bg">
	...
	$(function() {
		...
		var colNames = ['','','','','','检测项目','任务编号','单位名称','检测性质','样品编号','样品名称','检测截止时间','上报截止时间','操作'];
		var colModel = [ 
			{
				name : 'isDelete',
				index : 'isDelete',
				...
				name : 'samplingVo.sampName',
				index : 'sampling.SampName',
				width : 100,
				sortable : false,
				search : false,
				sortable : false
	...
	function gridComplete() {
		...
		for (var i = 0; i < ids.length; i++) {
			...
			if(rData.isDelete=='true'){
				be='<a class="btn btn-outline btn-danger btn-xs" title="删除" href="javascript:deleteOne(\''+ids[i]+'\')">删除</a> ';
			}
			be += '<a class="btn btn-outline btn-success btn-xs" title="修改" href="javascript:openEditPage(\'editBatch.do?ids='+ids[i]+'\')">编辑</a>';
