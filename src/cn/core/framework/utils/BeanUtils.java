...
public abstract class BeanUtils extends org.springframework.beans.BeanUtils {
	...
	public static void copyProperties(Object source, Object target,
			String... ignoreProperty) throws BeansException {
		BeanUtils.copyProperties(source, target, null, ignoreProperty);
	}

	public static void copyProperties(Object source, Object target,
			PropertyConvert pc, String... ignoreProperty) throws BeansException {
		copyProperties(1, source, target, pc, ignoreProperty);
	}

	public static void copyProperties(int level, Object source, Object target,
			PropertyConvert pc, String... ignoreProperty) throws BeansException {
		...
		for (PropertyDescriptor targetPd : targetPds) {
			if (isIgnoreProperty(targetPd.getName(), ignoreProperty))
				continue;
			...
			if (targetPd.getWriteMethod() != null) {
				...
				if (null == sourcePd) {
					...
					if (null != pc) {
						targetPdName = pc.getTargetPropertyName(targetPd
								.getName());
					}
					if (targetPdName == null) {
						if (targetPd.getName().endsWith("Vo")) {
							targetPdName = targetPd.getName().substring(0,
									targetPd.getName().indexOf("Vo"));
						} else {
							targetPdName = targetPd.getName() + "Vo";
						}
					}
					sourcePd = getPropertyDescriptor(source.getClass(),
							targetPdName);
				}
						...
						if (!sourcePd.getPropertyType().equals(
							...
							if (targetPd.getPropertyType().equals(String.class)) {
								value = String.valueOf(value);
									...
									.isPrimitiveOrWrapper(targetPd
											.getPropertyType())
									&& !ClassUtils
											.isPrimitiveOrWrapper(sourcePd
													.getPropertyType())) {
								Object targetValue = targetPd.getReadMethod()
										.invoke(target);
								if (null == value) {
									continue;
								}
								if (null == targetValue) {
									if (!List.class.isAssignableFrom(targetPd
											.getPropertyType())
											&& !Map.class
													.isAssignableFrom(targetPd
															.getPropertyType())
											&& !Set.class
													.isAssignableFrom(targetPd
															.getPropertyType())
											&& !targetPd.getPropertyType()
													.isArray()) {
										targetValue = targetPd
												.getPropertyType()
												.newInstance();
										targetPd.getWriteMethod().invoke(
												target, targetValue);
									} else {
										continue;
									}
								...
								if (level < 5) {
									copyProperties(level + 1, value,
											targetValue, pc, ignoreProperty);
								}
								continue;
							} else {
								continue;
	...
	private static boolean isIgnoreProperty(String property,
