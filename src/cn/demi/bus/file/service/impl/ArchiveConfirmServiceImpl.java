...
@Service("bus.archiveConfirmService")
public class ArchiveConfirmServiceImpl extends BaseServiceImpl<ArchiveVo, Archive> implements IArchiveConfirmService {
	@Autowired
	...
	public GridVo gridData(GridVo gridVo, ArchiveVo v) throws GlobalException {
		...
		pageResult.addCondition("status", QueryCondition.EQ, Archive.STATUS_SPTG);
		pageResult.addCondition("confirmId", QueryCondition.EQ, getCurrent().getAccountId());
	...
	public GridVo gridDatad(GridVo gridVo, ArchiveVo v) throws GlobalException {
		PageResult pageResult = getPageResult(gridVo, v);
		pageResult.addCondition(new QueryCondition("confirmTime is not null"));
		pageResult.addCondition(new QueryCondition("confirmId", QueryCondition.EQ, getCurrent().getAccountId()));
		if (StrUtils.isBlankOrNull(gridVo.getSidx()) || "lastUpdTime".equals(gridVo.getSidx())) {
			pageResult.addOrder("sort", OrderCondition.ORDER_ASC);
		} else {
			pageResult.setOrder(gridVo.getSord());
	...
	public void update(ArchiveVo v) throws GlobalException {
		...
		if (null!=v.getIsCommit() && v.getIsCommit().equals(Constants.PASS_Y)) {
			p.setStatus(Archive.STATUS_YQR);
			progressDao.update(pg.getId(), EunmTask.GD_CY.getStatus(),p.getOrgId(),p.getOrgName(),p.getConfirmId(), p.getConfirm());
		}
		progressLogDao.add(p.getId(), p.getId(), EunmTask.GD_QR.getStatus(), v.getIsCommit(), p.getConfirmCt());
		archiveDao.update(p);
	}
