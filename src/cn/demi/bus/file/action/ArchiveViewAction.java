 ...
 *
 */
@Controller("bus.archiveViewAction")
@RequestMapping("/bus/archiveView")
public class ArchiveViewAction extends BaseAction<ArchiveVo> {
	final String VIEW_PATH = "/bus/archive/file/view/archive";
	@Autowired
	private IArchiveViewService archiveViewService;
	@Autowired
	...
	public ModelAndView edit(ArchiveVo v) throws GlobalException {
		...
		if (null != v && !StrUtils.isBlankOrNull(v.getId())) {
			v = archiveViewService.findById(v.getId());
		} else {
	...
	@ResponseBody
	@Log(operation = Log.Operation.UPDATE, content = "查阅通过", module = "归档管理")
	public Status updateData(ArchiveVo v, RedirectAttributes attr) throws GlobalException {
		try {
			archiveViewService.update(v);
