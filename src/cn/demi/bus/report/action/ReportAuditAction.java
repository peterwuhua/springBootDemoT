 ...
 * @author QuJunLong
 */
@Controller("bus.reportAuditAction")
@RequestMapping("/bus/reportAudit")
public class ReportAuditAction extends BaseAction<ReportVo> {
	final String VIEW_PATH = "/bus/report/audit/report_audit";
	@Autowired 
	private IReportAuditService reportAuditService;	
	@Autowired 
	...
	public ModelAndView edit(ReportVo v) throws GlobalException {
		...
		if(null!=v && !StrUtils.isBlankOrNull(v.getId())){
			v=reportAuditService.findById(v.getId());
		}
	...
	public ModelAndView show(ReportVo v) throws GlobalException {
		...
		mav.addObject(VO, v);
		ProgressLogVo logVo=progressLogService.find(v.getTaskVo().getId(), EunmTask.TASK_SH.getStatus());
