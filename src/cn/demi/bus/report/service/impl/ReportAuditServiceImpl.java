...
import cn.demi.bus.report.service.IReportAuditService;
...
@Service("bus.reportAuditService")
public class ReportAuditServiceImpl extends BaseServiceImpl<ReportVo,Report> implements
		IReportAuditService {
	...
	public void update(ReportVo v) throws GlobalException {
		 ...
		 if(!StrUtils.isBlankOrNull(v.getSignUserId())) {
			Account at=accountDao.findById(v.getSignUserId());
			v.setSignUser(at.getUser().getName());
		 ...
		 if(v.getIsCommit().equals(EunmTask.PASS_Y)) {
			 if(!StrUtils.isBlankOrNull(v.getSignUserId())) {
				pg=progressDao.update(pg.getId(),EunmTask.TASK_PZ.getStatus(),v.getSignUserId(),v.getSignUser());
			 }else {
				 pg=progressDao.update(pg.getId(),EunmTask.TASK_PZ.getStatus());
			 }
		...
		reportDao.update(po);
		progressLogDao.add(po.getTask().getId(),po.getTask().getId(),EunmTask.TASK_SH.getStatus(),v.getIsCommit(),v.getAuditMsg());
		Task task=po.getTask();
		task.setStatus(po.getStatus());
		taskDao.update(task);
		sRecordDao.insert(po.getTask().getNo(),po.getMakeDate(),Constants.KH_RW_BGSH);
	 }
	...
	public GridVo gridData(GridVo gridVo, ReportVo v) throws GlobalException {
		...
		pageResult.addCondition(new QueryCondition("status='"+EunmTask.TASK_SH.getStatus()+"' " ));
		pageResult.addCondition(new QueryCondition("auditUserId like '"+CurrentUtils.getCurrent().getAccountId()+"%' or auditUserId is null or auditUserId='' "));
	...
	public GridVo gridDatad(GridVo gridVo, ReportVo v) throws GlobalException {
		...
		hql.append(" AND log.status='"+EunmTask.TASK_SH.getStatus()+"' AND log.userId like '"+CurrentUtils.getCurrent().getAccountId()+"' ");
