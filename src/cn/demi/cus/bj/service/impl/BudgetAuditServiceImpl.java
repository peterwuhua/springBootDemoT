...
@Service("cus.budgetAuditService")
public class BudgetAuditServiceImpl extends BaseServiceImpl<BudgetVo,Budget> implements
	IBudgetAuditService {
	@Autowired
	...
	public void update(BudgetVo v) throws GlobalException {
		...
		}else if(StrUtils.isBlankOrNull(v.getRemark())){
			p.setRemark(p.getRemark()+";"+v.getRemark());
		}
		if(v.getIsCommit().equals(Constants.PASS_Y)) {
			p.setStatus(Budget.ST_2);
		}else {
			p.setStatus(Budget.ST_0);
			p.setBack(Constants.Y);
		}
		budgetDao.update(p);
	}
	...
	public GridVo gridData(GridVo gridVo, BudgetVo v) throws GlobalException {
		...
		pageResult.addCondition(new QueryCondition("status='"+Budget.ST_1+"' "));
