...
@Service("office.kqAuditService")
public class KqAuditServiceImpl extends BaseServiceImpl<KqVo,Kq> implements
		IKqAuditService {
	@Autowired
	...
	public void update(KqVo v) throws GlobalException {
		...
		kqDao.update(po);
		progressLogDao.add(po.getId(), po.getId(), EunmTask.QJ_AD.getStatus(), v.getIsCommit(), v.getAuditMsg(),getCurrent().getDepId(), getCurrent().getDepName(),getCurrent().getAccountId(),getCurrent().getUserName());
	}
	...
	public GridVo gridData(GridVo gridVo, KqVo v) throws GlobalException {
		...
		pageResult.addCondition(new QueryCondition("orgId", QueryCondition.EQ,getCurrent().getOrgId()));
		pageResult.addCondition(new QueryCondition("status", QueryCondition.EQ, EunmTask.QJ_AD.getStatus()));
