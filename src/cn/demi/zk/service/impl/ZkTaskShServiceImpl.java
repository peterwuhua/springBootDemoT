...
@Service("zk.taskShService")
public class ZkTaskShServiceImpl extends BaseServiceImpl<ZkTaskVo,ZkTask> implements
		IZkTaskShService {
	@Autowired
	...
	public void update(ZkTaskVo v) throws GlobalException {
		...
		if(null!=v.getIsCommit()&&v.getIsCommit().equals(Constants.PASS_Y)) {
			p.setStatus(EunmZkTask.TASK_20.getStatus());
			progressLogDao.add(p.getId(),EunmZkTask.TASK_10.getStatus(),"通过",v.getRemark());
		}else if(null!=v.getIsCommit()&&v.getIsCommit().equals(Constants.PASS_N)){
			p.setStatus(EunmZkTask.TASK_00.getStatus());
			p.setIsBack(Po.Y);
			progressLogDao.add(p.getId(),EunmZkTask.TASK_10.getStatus(),"退回",v.getRemark());
		}
		zkTaskDao.update(p);
	}
	@Override
