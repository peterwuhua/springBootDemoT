...
@Service("zk.taskPzService")
public class ZkTaskPzServiceImpl extends BaseServiceImpl<ZkTaskVo,ZkTask> implements
		IZkTaskPzService {
	@Autowired
	...
	public void update(ZkTaskVo v) throws GlobalException {
		...
		for (ZkSamplingVo sampVo : sampList) {
			...
			samp.setSampDate(sampVo.getSampDate());
			samp.setSampCode(sampVo.getSampCode());
			samp.setTj(sampVo.getTj());
			samp.setFfgc(sampVo.getFfgc());
			samp.setPnd(sampVo.getPnd());
			samp.setCnd(sampVo.getCnd());
			samp.setHsl(sampVo.getHsl());
			samp.setJmd(sampVo.getJmd());
			samp.setCuserId(sampVo.getCuserId());
			samp.setCuserName(sampVo.getCuserName());
			samp.setMuserId(sampVo.getMuserId());
			samp.setMuserName(sampVo.getMuserName());
			samp.setRemark(sampVo.getRemark());
			zkSamplingDao.update(samp);
		}
		if(null!=v.getIsCommit()&&v.getIsCommit().equals(Constants.PASS_Y)) {
			p.setStatus(EunmZkTask.TASK_30.getStatus());
			zkTaskDao.update(p);
			progressLogDao.add(p.getId(),EunmZkTask.TASK_20.getStatus(),"提交",v.getRemark());
		}
	}
	@Override
	public ZkTaskVo findById(String id) throws GlobalException {
		...
		if(null!=sampList) {
			for (ZkSampling samp : sampList) {
				...
				if(StrUtils.isBlankOrNull(sampVo.getMuserId())) {
					sampVo.setMuserId(getCurrent().getAccountId());
					sampVo.setMuserName(getCurrent().getUserName());
				}
				if(StrUtils.isBlankOrNull(sampVo.getSampDate())) {
					sampVo.setSampDate(DateUtils.getCurrDateStr());
				}
