...
@Service("qlt.nsReportService")
public class NsReportServiceImpl extends BaseServiceImpl<NsRecordVo,NsRecord> implements
		INsReportService {
	@Autowired
	...
	public void update(NsRecordVo v) throws GlobalException {
		...
		if(null!=v.getIsCommit() && v.getIsCommit().equals(QltEunm.PASS_Y)) {
			Progress pg=progressDao.update(p.getPg().getId(),QltEunm.QLT_M_40.getStatus());
			p.setStatus(pg.getStatus());
			nsRecordDao.update(p);
			progressLogDao.add(p.getNs().getId(), p.getId(),QltEunm.QLT_M_30.getStatus(),v.getIsCommit(),v.getRemark());
		}
	}
	...
	public void update4Report(NsRecordVo v) throws GlobalException {
		NsRecord po  = nsRecordDao.findById(v.getId());
		po.setFileName(v.getFileName());
		po.setFilePath(v.getFilePath());
		nsRecordDao.update(po);
