...
@Service("qlt.nsVerifyService")
public class NsVerifyServiceImpl extends BaseServiceImpl<NsRecordVo,NsRecord> implements
		INsVerifyService {
	@Autowired
	...
	public void update(NsRecordVo v) throws GlobalException {
		...
		p.setYzId(v.getYzId());
		p.setYzName(v.getYzName());
		p.setYzDate(v.getYzDate());
		...
		if(null!=v.getIsCommit() && v.getIsCommit().equals(QltEunm.PASS_Y)) {
			Progress pg=progressDao.update(p.getPg().getId(),QltEunm.QLT_M_30.getStatus());
			p.setStatus(pg.getStatus());
			nsRecordDao.update(p);
			progressLogDao.add(p.getNs().getId(), p.getId(),QltEunm.QLT_M_20.getStatus(),v.getIsCommit(),v.getRemark());
		}
	}
